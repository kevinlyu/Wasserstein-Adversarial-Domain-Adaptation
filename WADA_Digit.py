...
class WADA_II:
    ...
    def train(self):
        ...
        for epoch in range(self.total_epoch):
            ...
            for index, (src, tar) in enumerate(zip(self.src_loader, self.tar_loader)):
                ...
                loss = 10*class_loss + alpha*rs.mean()*wasserstein_distance
                ...
                for _ in range(self.d_iter):
                    ...
                    domain_loss = -rs.mean()*alpha*wasserstein_distance + 10*gp
                    d_opt.zero_grad()
    ...
    def save_model(self, path="./saved_WADA_Digit/"):
        try:
            os.stat(path)
        except:
            os.mkdir(path)
    ...
    def load_model(self, path="./saved_WADA_Digit/"):
    ...
    def visualize(self, dim, plot_num):
        ...
        if dim == 2:
            visualize_2d("./saved_WADA_Digit/", embedding,
                         label, tag, self.class_num)

        elif dim == 3:
            visualize_3d("./saved_WADA_Digit/", embedding,
                         label, tag, self.class_num)
...
if __name__ == "__main__":
    ...
    src_extractor = Extractor_Digit().cuda()
    tar_extractor = Extractor_Digit().cuda()

    classifier = Classifier_Digit(class_num=class_num).cuda()

    relater = Relater_Digit().cuda()
    discriminator = Discriminator_Digit().cuda()

    class_criterion = nn.CrossEntropyLoss()
    relation_criterion = nn.BCELoss()

    #Adam , RMSprop, RMSprop
